...
public class Tetris2048 {
	public static void main(String[] args) throws CloneNotSupportedException {
		...
		while (true) {
			...
			while (game && !menu) { // game loop
				...
				// TETRIS CONTROL
				if (tetrisClear) {
					boolean[] isTetris = isThereTetris(canvas, tetrisScore); 													// tetris control for every row in the table
	 ...
	 * @return returns boolean array for determining whether is it full horizontal
	 *         row or not in the current table
	 */
	public static boolean[] isThereTetris(CanvasDrawer canvas, int tetrisScore) {
	...
	/**
	 * Erases row if the row is tetris Moves every row above the tetris down one
	 * line. Returns true if there was a tetris
	 * 
	 * @param currentTable stores tables last status in types of 1 or 0.
	 * @param isTetris     boolean array which contains every row's status of being
	 *                     tetris or not
